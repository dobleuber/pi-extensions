...
class RogerCliTests(unittest.TestCase):
    ...
    def test_health_command_reports_core_configuration(self):
        ...
        self.assertIn("tts voice: spanish-default", output)
        self.assertIn("tts speed: 1.0", output)
        self.assertIn("tts split pattern: (backend default)", output)
        self.assertIn("tts supported prosody: voice, voice blend, speed, split pattern", output)
        self.assertIn("tts unsupported prosody: direct emotion/tone/style controls", output)
