...
Roger SHALL decide whether each pi-agent task should use pi's configured online default model or the configured Ollama fallback before dispatch.
...
- **THEN** Roger SHALL skip online provider probing and SHALL use the configured Ollama fallback launch profile
...
- **THEN** Roger SHALL use the configured Ollama fallback launch profile for the task
...
- **THEN** Roger SHALL retry the task once using the configured Ollama fallback session

#### Scenario: Non-provider task failure
- **WHEN** pi-agent accepts the task and later reports a normal task/tool error
- **THEN** Roger SHALL NOT automatically rerun the task on Ollama unless the user explicitly asks

### Requirement: Offline fallback unavailable reporting
Roger SHALL visibly and audibly report when offline execution cannot be used because Ollama or the configured fallback model is unavailable.

#### Scenario: Ollama server unavailable
- **WHEN** Roger needs offline fallback and cannot connect to the Ollama server
...
- **WHEN** Roger needs offline fallback and the configured Ollama model is not installed
...
- **WHEN** Roger dispatches a task using the Ollama fallback profile
