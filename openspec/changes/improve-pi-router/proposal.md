...
- Use a concise conversation summary only to resolve references in the current prompt, while keeping translation faithful to the user's latest message.
...
- Add an on-demand router details toggle, similar in spirit to Pi's expandable/collapsible output controls, so the user can inspect the translated English prompt before work-model dispatch and inspect the final Spanish/English answers afterward.
- Treat prompt/answer inspection as a UI toggle, not as a separate router mode; routing mode stays focused on enabled/disabled behavior.
- Add safe fallback behavior when local translation, thinking classification, final-answer translation, or router details UI is unavailable, so Pi can either proceed transparently or explain the degraded mode instead of silently dropping the task.
...
- `pi-router-model-policy`: Configurable router/work-model policy that keeps local `gemma4` available for routing, supports Stratus as a normal work-model target once credits are available, and exposes router enable/disable plus inspection controls.
