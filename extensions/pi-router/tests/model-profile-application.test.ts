...
import { ASTRA_HIGH_PROFILE, applyModelProfileToRuntime, type ModelProfileRuntime } from "../src/model-profile.ts";
...
describe("router model profile application", () => {
	it("resolves the profile through the registry and applies model and thinking atomically", async () => {
		...
		const result = await applyModelProfileToRuntime({ ...ASTRA_HIGH_PROFILE, source: "prompt" }, runtime);
	...
	it("rejects a registry result whose identity does not match the requested profile", async () => {
		...
		const result = await applyModelProfileToRuntime({ ...ASTRA_HIGH_PROFILE, source: "prompt" }, {
