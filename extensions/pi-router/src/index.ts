...
export function installPiRouter(pi: ExtensionAPI, dependencies: PiRouterDependencies = {}) {
	...
	let lastRogerSpeechRequest = false;
	...
	pi.on("message_end", async (event, ctx) => {
		...
		const replacementText = lastRogerSpeechRequest
			? buildRogerSpeechResponse(translated.englishAnswer, translated.spanishAnswer)
			: translated.spanishAnswer;
		lastRogerSpeechRequest = false;
		return {
			message: {
				...event.message,
				content: replaceTextContent(event.message.content, replacementText),
			},
		};
	});

	pi.on("input", async (event, ctx) => {
		lastRogerSpeechRequest = isRogerSpeechRequest(event);
