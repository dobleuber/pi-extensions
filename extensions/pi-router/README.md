...
| --- | --- | --- | --- |
| **Luna Max** (default) | `openai-codex/gpt-6-luna` | `max` | Clearly scoped implementation, bug fixes, tests, documentation, reviews, bounded refactors, and moderately complex work with clear acceptance criteria—even across multiple files. Not limited to trivial changes; no low-latency claim. |
| **Sol** | `openai-codex/gpt-6-sol` | `xhigh` | Demanding programming tasks and multi-step agentic workflows—complex implementations, difficult debugging, and work requiring sustained tool use—when Sol offers better performance per cost than Astra Low. |
| **Astra Medium** | `openai-codex/gpt-6-astra` | `medium` | Reserved for novel architecture, deeply ambiguous requirements, or interacting cross-system constraints that justify choosing Astra over Sol. Complexity or sustained tool use alone favors Sol. |
...
Use Sol: trace and fix this intermittent lifecycle race
Usa Sol: investiga y corrige esta carrera intermitente del ciclo de vida
...
Astra can be selected by Jev or an explicit directive when choosing it over Sol is justified. Pricing and performance vary by model and workload; check current provider information before use. The Sol criterion describes the intended performance-per-cost trade-off over Astra Low, not a guarantee for every task.

Model and task assignments are application policy categories, not OpenAI benchmarks. Model references: [Luna](https://developers.openai.com/api/docs/models/gpt-6-luna), [Sol](https://developers.openai.com/api/docs/models/gpt-6-sol), and [Astra](https://developers.openai.com/api/docs/models/gpt-6-astra). Luna retains max reasoning and Sol uses xhigh reasoning.
