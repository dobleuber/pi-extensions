...
Roger separates canonical written text from TTS speech text. Preview, overlay, CLI, task logs, and retry/debug context keep correct spelling such as `README`, `GitHub`, `JSON`, paths, commands, and Markdown. With TTS enabled, Roger sends pi RPC metadata like `{"source":"roger","speech":{"enabled":true,"language":"es"}}`; pi-router may answer with `display_text`, `speech_text`, `speech_language`, and `speech_source`. Roger displays/logs `display_text` and sends only `speech_text` to Kokoro. Model-backed speech naturalization goes through pi-agent/pi-router and the configured pi provider/model routing, including offline `llama-cpp` fallback; Roger does not call llama.cpp directly for speech naturalization. If structured speech fields are absent or invalid, Roger uses deterministic local cleanup for Markdown, times, and common anglicisms without changing task status. The final string sent to Kokoro uses Kokoro/Misaki pronunciation markup for known technical terms and Anglicisms, for example `[README](/ɹˈiːdmi/)` and `[GitHub](/ɡˈɪthʌb/)`; UI text remains canonical, while task speech logs record the marked-up Kokoro input.
