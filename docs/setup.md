...
Roger separates canonical written text from TTS speech text. Preview, overlay, CLI, task logs, and retry/debug context keep correct spelling such as `README`, `GitHub`, `JSON`, paths, commands, and Markdown. The TTS input may be rewritten for pronunciation, and spoken prose must be Spanish except for technical Anglicisms, product names, commands, paths, and code identifiers. For example, `Son las **10:30 am**. Revisa el README en GitHub.` can be spoken as “Son las diez y treinta de la mañana. Revisa el ridmi en guit jab.” By default, the preferred naturalizer is local Gemma through the llama.cpp OpenAI-compatible endpoint (`http://127.0.0.1:11434/v1`, model `gemma4`); if that endpoint is unavailable or times out, Roger uses deterministic local cleanup for Markdown, times, and common anglicisms without changing task status. Set `[speech.naturalization] enabled = false` to disable the Gemma translation step and use only deterministic cleanup.
