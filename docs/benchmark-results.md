...
On Omarchy/Hyprland, the overlay uses `gtk-layer-shell` as the definitive Wayland-native feedback surface, with Tk fallback only when layer-shell is unavailable; duplicate `notify-send` notifications are opt-in via `--desktop-notifications`
```
